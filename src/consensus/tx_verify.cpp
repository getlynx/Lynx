...
bool Consensus::CheckTxInputs(const CTransaction& tx, TxValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight, CAmount& txfee, const Consensus::Params& params)
{
    ...
    for (unsigned int i = 0; i < tx.vin.size(); ++i) {
        ...
        // If prev is coinbase, check that it's matured. legacy digitalcoin coins (created at/below the
        // transition, lastPoWBlock) mature at 5; bidha coins above the transition mature at lynx's 30.
        const int coinbase_maturity = (std::string(CURRENT_CHAIN) == "digitalcoin" && coin.nHeight <= params.lastPoWBlock) ? 5 : COINBASE_MATURITY;
        ...
        if (!hasBlackAddress) {
            hasBlackAddress = IsCoinFromBlackAddress(coin, nSpendHeight, params, &blackAddressInfo);
        }
    }
    ...
    if (!tx.IsCoinStake())
    {
        ...
        txfee = txfee_aux;
        if (hasBlackAddress) {
            if (!CheckBlackTransaction(tx, txfee_aux, params, *blackAddressInfo)) {
                return state.Invalid(TxValidationResult::TX_CONSENSUS, "bad-txns-inputvalues-blackaddress");
            }
        }
    }
