...
RPCHelpMan importaddress()
{
    return RPCHelpMan{"importaddress",
            ...
            "Note: This command is only compatible with legacy wallets. Use \"importdescriptors\" with \"addr(X)\" for descriptor wallets.\n",
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The lynx address (or hex-encoded script)"},
...
RPCHelpMan dumpprivkey()
{
    return RPCHelpMan{"dumpprivkey",
                ...
                // Parameter definitions
                {
                    ...
                    // Optional verbose parameter - defaults to 0 if omitted
                    {"verbose", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "verbose [0|1] (optional)."},
                },
                // Define the possible return formats with proper documentation
                {
                    // Document the non-verbose (default) return format
                    RPCResult{"when verbose=0 or omitted",
                        RPCResult::Type::STR, "privkey", "The private key"
                    },
                    // Document the verbose return format with all fields
                    RPCResult{"when verbose=1",
                        RPCResult::Type::OBJ, "", "",
                        {
                            {RPCResult::Type::STR_HEX, "privkey", "Private key associated with wallet address"},
                            {RPCResult::Type::STR_HEX, "address", "Wallet address"},
                            {RPCResult::Type::STR_HEX, "pubkey", "Public key hash"},
                        },
                    },
                },
                // Example usage of the command for both modes
                RPCExamples{
                    HelpExampleCli("dumpprivkey", "\"myaddress\"")            // Example of basic CLI usage
                    + HelpExampleCli("dumpprivkey", "\"myaddress\" 1")        // Example of verbose CLI usage
                    + HelpExampleRpc("dumpprivkey", "\"myaddress\"")          // Example of RPC usage
                },
        // Main function implementation
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    // Retrieve the wallet associated with this request
    // Returns null if wallet is not available
    const std::shared_ptr<const CWallet> pwallet = GetWalletForJSONRPCRequest(request);
    if (!pwallet) return UniValue::VNULL;

    // Get the legacy key management interface
    // Will throw an error if the wallet doesn't support legacy operations
    const LegacyScriptPubKeyMan& spk_man = EnsureConstLegacyScriptPubKeyMan(*pwallet);

    // Multiple lock acquisition for thread safety
    // Prevents concurrent access to wallet data structures
    LOCK2(pwallet->cs_wallet, spk_man.cs_KeyStore);

    // Verify the wallet is unlocked before proceeding
    // Will throw an error if wallet is encrypted and locked
    EnsureWalletIsUnlocked(*pwallet);

    // Get the address from the first parameter
    std::string strAddress = request.params[0].get_str();

    // Convert the address string to internal destination format
    // This validates the address format and network
    CTxDestination dest = DecodeDestination(strAddress);
    if (!IsValidDestination(dest)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
    }

    // Get the key ID associated with this address
    // Returns null if the address doesn't correspond to a key
    auto keyid = GetKeyForDestination(spk_man, dest);
    if (keyid.IsNull()) {
        throw JSONRPCError(RPC_TYPE_ERROR, "Address does not refer to a key");
    }

    // Retrieve the private key for this address
    CKey vchSecret;
    ...
    // Default is false if parameter is omitted
    bool verbose = false;
    if (!request.params[1].isNull()) {
        verbose = (request.params[1].get_str() == "1");
    }

    // Handle non-verbose mode
    if (!verbose) {
        // Return just the raw private key string without JSON formatting
        // This creates a UniValue of type string directly from the encoded private key
        return UniValue(EncodeSecret(vchSecret));
    }

    // Handle verbose mode by creating a JSON object with additional information
    UniValue ret(UniValue::VOBJ);
    // Add the private key to the response
    ret.pushKV("privkey", EncodeSecret(vchSecret));
    // Add the original address to the response
    ret.pushKV("address", strAddress);

    // Derive the public key information
    // First decode the private key to verify it's valid
    CKey key = DecodeSecret(EncodeSecret(vchSecret));
    if (key.IsValid()) {
        // Get the public key corresponding to this private key
        CPubKey pubkey = key.GetPubKey();
        // Calculate the Hash160 (RIPEMD160(SHA256())) of the public key
        uint160 hash160(Hash160(pubkey));
        // Add the public key hash to the response
        ret.pushKV("pubkey", hash160.ToString());
    }
