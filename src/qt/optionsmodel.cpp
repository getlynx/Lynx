...
/** Call node.updateRwSetting() with Lynx 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Lynx 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/lynx/lynx/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because lynx 22.x will not try to read these.
...
//! Convert enabled/size values to lynx -prune setting.
...
//! Get pruning enabled value to show in GUI from lynx -prune setting.
...
//! Get pruning size value to show in GUI from lynx -prune setting. If
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if lynx.conf value
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/lynx/lynx/pull/8273
    ...
    // (https://github.com/lynx-core/gui/issues/567).
