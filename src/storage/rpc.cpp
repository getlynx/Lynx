        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    // 
    // When the tenant executes a 'lynx-cli store' command, the tenant presents a fully qualified asset filename. 
    // The daemon uses the fully qualified asset filename to read the asset contents from the hard drive, 
    // then discards the fully qualified asset filename.
    //
    // Before storing the asset on the blockchain, the daemon generates a new Universally Unique IDentifier (UUID), 
    // where universally unique applies to the blockchain. 
    // The UUID will always be 64 hex digits in length. 
    // The UUID is generated by concatenating 32 two- hex-digit representations of a randomly generated 8-bit integer. 
    // We call this UUID the unobfuscated UUID.
    //
    // Next, the daemon encrypts hex digits 9-41 numbering from the left, beginning with 1. 
