#!/bin/bash
# rpcpatch.sh - Patch chain conf file with per-chain RPC loopback address
#
# Called by the {chain}-rpcpatch.timer systemd unit. Expects these
...
for var in CONF_PATH RPC_HOST SERVICE_NAME TIMER_UNIT; do
    if [ -z "${!var:-}" ]; then
        logger -t rpcpatch "ERROR: $var not set. Exiting."
        exit 1
    fi
done
...
if [ ! -f "$CONF_PATH" ]; then
    logger -t rpcpatch "Conf file $CONF_PATH not found. Will retry."
    exit 0
fi
...
logger -t rpcpatch "Updated main.rpcbind and main.rpcallowip to $RPC_HOST in $CONF_PATH."
...
systemctl stop "$TIMER_UNIT" 2>/dev/null || true
systemctl disable "$TIMER_UNIT" 2>/dev/null || true
logger -t rpcpatch "Disabled $TIMER_UNIT."
